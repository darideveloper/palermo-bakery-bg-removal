"""
Description:
    This script takes the transparent PNG images generated by the background removal script,
    adds a solid white background, and saves them in their original file format (e.g., .jpg) 
    in a new directory ('uploads-no-bg-done').

How it works:
    1. Scans the 'uploads-no-bg' directory for transparent PNGs.
    2. Uses an SQLite database ('finalizing_status.db') to track progress.
    3. For each PNG, it finds the corresponding original file in the 'uploads' directory 
       to determine the correct target file extension.
    4. Composites the transparent PNG over a white background.
    5. Saves the final image to 'uploads-no-bg-done', matching the original extension.

Related Folders:
    - Source: ./uploads-no-bg (transparent images)
    - Reference: ./uploads (to match original extensions)
    - Output: ./uploads-no-bg-done (final white background images)
    - DB: finalizing_status.db

Related Scripts:
    - remove-bg.py (MUST be run before this script to generate the transparent PNGs)
    - remove-scaled-suffix.py (typically run after this to finalize filenames)

Recommended Run Order:
    1. remove-bg.py
    2. add-white-bg.py (This script)
    3. remove-scaled-suffix.py

Example Input:
    - 'uploads-no-bg/2026/05/cake.png' (transparent) + original extension from 'uploads/2026/05/cake.jpg'

Example Output:
    - 'uploads-no-bg-done/2026/05/cake.jpg' (white background, original extension)
"""
import os
import sqlite3
import time
